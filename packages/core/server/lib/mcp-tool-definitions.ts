    ...
    inputSchema: objectSchema(
      {
        ...
        data: anyObjectSchema(
          'Template data payload for a template_id-driven PDF render. To bind an image supplied in `assets.images`, the slot value must be that asset\'s BARE assetId (e.g. "cover") — pdf-tool prefixes it to "https://render.assets.invalid/<assetId>" itself, exactly once, per the template\'s own form; a full virtual URL written into a slot a template already prefixes is refused with ASSET_REFERENCE_DOUBLED. When a slot the template uses as an image source holds something the renderer cannot resolve, the render fails the referenced-asset precheck with ASSET_MISSING naming the SLOT, not the assetId — which is what to read when an asset you did supply looks missing. A raw data: URI written straight into a slot value also renders, but inlines the bytes into the render payload.'
        ),
        assets: anyObjectSchema(
          'Optional supporting assets for a template_id-driven PDF render: {images: [{assetId, dataUri} | {assetId, blobKey}]}. BOTH entry shapes resolve — a blobKey is fetched from this site\'s artifact store server-side, a dataUri is inlined — and each is addressed from the template\'s own markup as "https://render.assets.invalid/<assetId>", which the renderer serves off its virtual host; a `data` slot value referencing this asset should supply the bare assetId, not that virtual URL (see `data` above). An assetId the template references but that is not declared here fails the render with ASSET_MISSING before any page is drawn.'
        ),
