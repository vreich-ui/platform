...
} from './quick-actions-registry.js';

/**
 * ASV2-W5 — the registry DATA and the types it is shaped from now live in the
 * leaf `quick-actions-registry.ts`, and every one of them is re-exported
 * here, so nothing that imports this module changed. The split exists purely
 * so the SERVER's `ui-capabilities.ts` can read `QUICK_ACTIONS` without
 * dragging this file's execution half into `admin-agent-chat`'s cold start —
 * see that module's header and the bundle-budget test's "THE CUT".
 */
export {
  QUICK_ACTIONS,
  type QuickActionChoiceField,
  type QuickActionDefinition,
