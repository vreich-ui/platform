        ...
        : objectType === 'section_template'
          ...
          : objectType === 'theme'
            ...
            : objectType === 'tracking_config'
              ? 'The per-project tracker registry singleton (W13, 12-plan §3): fixed-key provider blocks with regex-pinned IDs (never script/URLs), the consent posture (geo-adaptive seed per OQ-W13-1), and the per-type collection defaults matrix. Publishing it is the site-wide script switch: the export (the site export root plus tracking.json — W11 T11.6 parameterized the export root per site) decides which trackers execute on every page. Human/seed-minted; agents edit via set_tracking_config_fields. It also carries experiments[] (T21.5): concurrent A/B arms served at the edge under ONE url — object_id is the CONTROL content_item, arms lists the complete served set (the control itself plus object_create_variant children of it, each with its published route), and only status:\'active\' serves. Setting status:\'concluded\' reverts every reader to the control on the next build.'
