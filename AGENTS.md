...
| **Membership verbs are human-only** (reads included); every Netlify Identity **e-mail/hash token** (invite · recovery · confirmation · email-change) lands on `/admin/accept`; session JWTs are consumed by the `admin-*` functions via `admin-auth.ts` | `handleMembershipVerb`'s first line 403s `membership_requires_human` for any agent principal — for `list` as much as for `purge`; never consume an Identity e-mail token elsewhere | `docs/cms-architecture/18-membership-plan.md`; gate at `packages/core/server/lib/membership/verbs.ts:332` |
...
| **The publishing-plugin charter is never widened** | The charter is *derived*, not hand-kept: tool classes `read`/`draft`/`creation`/`publication`, plus `release_to_production` by name, minus `PLUGIN_TOOL_DENYLIST`. Adding a class, adding a name to the privileged allow-list or removing a denylist entry needs a ruling. Since W0 (D4, 2026-09-11) it is a **permission boundary on both doors** — 403 `tool_not_in_plugin_charter` on `/api/plugin/*` (`plugin-actions.ts`) and on `/mcp` for any `plugin:*` surface (`plugin/charter-gate.ts`), plus `object_type_not_in_plugin_charter` for creating `template`/`section_template`/`theme`/`site`. A tenant with no promoted manifest, or an unreadable manifest store, degrades the tool-name rule and logs `mcp_charter_unenforced` | `packages/core/server/lib/plugin/{build-tools,charter-gate}.ts` (constraint restated at `docs/plugin/recon-genesis.md:86`) |
...
7. **Never commit the literal value of `GITHUB_REPOSITORY`** — this repo's own `owner/name` — in any form (bare slug or inside a `https://github.com/…` URL), nor the plain-text value of any other configured env var: Netlify's build-time secrets scanner fails the build on a configured value appearing anywhere in scanned content (record: `docs/history/CLAUDE-2026-09-05.md:366`; mitigation `SECRETS_SCAN_OMIT_KEYS` in every `netlify.toml` is a backstop, not permission). Convention that follows: reference this repo's PRs/issues as `#NNN`, never as a URL; name third-party repositories as `owner/name` text rather than links. `tests/scripts/docs-invariants.test.mjs` enforces the slug rule on the architecture docs; `docs/history/**` is a verbatim archive excluded by design (`KNOWN_ISSUES.md` #64).
...
- `npm test` runs three stages: `tsc -p tsconfig.test.json` + `node --test` over `tests/netlify/**`, then `tests/scripts/*.test.mjs`, then `packages/core/cli/capture/*.test.mjs`. offline (~3 min; current counts in `docs/DEPLOYMENT.md` §Verification run log).
...
- **Docs are tested too.** `tests/scripts/docs-invariants.test.mjs` (links resolve, no sandbox paths, no repo slug, Mermaid blocks match `docs/diagrams/*.mmd`, no duplicate issue ids) and `tests/scripts/docs-inventory-fresh.test.mjs` (`docs/generated/INVENTORY.md` equals `node scripts/docs/inventory.mjs`). When a change adds a function, tool, schema, env var, tenant or namespace, run `node scripts/docs/inventory.mjs --write` in the same commit; never hand-edit the generated file or type counts into prose.
