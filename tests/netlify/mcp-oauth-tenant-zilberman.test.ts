/**
 * Fleet parity (P1): the whole OAuth connector flow, on the 'zilberman' tenant.
 *
...
import { handler as oauthHandler } from '../../sites/zilberman/netlify/functions/mcp-oauth.js';
import { handler as mcpHandler } from '../../sites/zilberman/netlify/functions/mcp.js';
import { runTenantOAuthFlow, type LambdaLikeHandler } from './tenant-oauth-flow.js';

const HOST = 'zilbermanfilmfoundation.netlify.app';
...
test('zilberman: register → authorize → consent → token → an authorized tools/list', async () => {
