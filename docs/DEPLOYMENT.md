...
1. **Ran** — Netlify UI → the project → **Logs → Functions → `<name>`**, and switch the log range from *Real-time* to *Last 7 days* (real-time shows an empty console and looks identical to "never ran"). A run that happened leaves the function's own JSON line plus a platform `Duration: … Memory Usage: …` line. `No results found for query` over 7 days on a daily job = it never ran. The page also has a **Run now** button, which is the only supported way to invoke a scheduled function by hand (over HTTP it answers 403).
