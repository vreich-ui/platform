...
- Four scheduled functions, identical schedules: `mcp-keepalive` (`*/5 * * * *`), `membership-sweep` (`17 3 * * *`), `media-compaction-sweep` (`41 3 * * *`), `editorial-request-sweep` (`*/5 * * * *`).
...
| `media-compaction-sweep` | `41 3 * * *` (daily) | **A DISPATCHER since 2026-09-16** — it mints a one-shot token and POSTs `media-compaction-sweep-background`, which does the work with a 15-minute budget. The sweep ran inline here until it was measured being killed by the 30 s scheduled-function wall on every single run (zilbermanfilmfoundation, receipt `startedAtISO` 09:29:12 with no `finishedAtISO` 95 s later), in the one-time collection phase that has no page boundary to checkpoint at. A function cannot be both scheduled and background, hence the pair. The work itself, W4: soft-deletes every live artifact reference no active object and no slot pointer cites (bytes kept — `restore_artifact` must stay possible), then collapses byte-duplicates onto the oldest blob per sha (`storageKey` only; `blobKey` is identity, so no `/img|/pdf` path moves). Runs the `sweepOrphanArtifacts` + `dedupeArtifactsBySha` libraries that until W4 were reachable only as the admin verbs `artifact_orphan_sweep` / `artifact_dedupe_by_sha`, which nobody ran. Two safeguards: a reference younger than **24 h** is never an orphan (capture writes artifacts BEFORE the pages that cite them, so a fresh upload is legitimately uncited for a while — those are counted `skipped_recent`), and dedupe runs AFTER the sweep on live references only, so one artifact is never pushed both directions in one run. Dangling references (an object citing an artifact that is not there) are REPORTED, never repaired. Neither pass writes a governed object, so nothing here flips a page to `unpublished_changes`. One JSON log line per run: `{ site, orphans_soft_deleted, dangling, dedupe_groups, blobs_deleted, bytes_freed, skipped_recent, stopped_early, resumed_from }`, and one receipt in the store at `maintenance/media-compaction-sweep.json` (see *Confirming a scheduled function actually RAN* below). A run stops paging after 20 s, checkpoints its cursors and finishes the cycle on the next invocation — a scheduled function is killed at 30 s and that kill leaves no log line at all. | The tenant's artifact store grows forever: orphaned references are never retired and duplicate blobs keep their own bytes. This is the mechanism, not an operator chore — nobody runs the verbs by hand (Wolf, 2026-09-13) |
...
#### Confirming a scheduled function actually RAN (2026-09-15)

A declared schedule is necessary and **not sufficient**. `media-compaction-sweep` shipped to all five projects on 2026-09-13 with the block declared, the shim in place, the parity audit green and Netlify's own deploy manifest carrying `function_schedules: media-compaction-sweep 41 3 * * *` — and then did not run once on any tenant for two days, with no log line and no error, while `membership-sweep` logged a clean run every morning from the same deploys. Declared ≠ registered ≠ ran. Check all three:

1. **Ran** — Netlify UI → the project → **Logs → Functions → `<name>`**, and switch the log range from *Real-time* to *Last 7 days* (real-time shows an empty console and looks identical to "never ran"). A run that happened leaves the function's own JSON line plus a platform `Duration: … Memory Usage: …` line — **for a function whose invocations complete**. Netlify logged NOTHING for `media-compaction-sweep` for three days while it was being killed at 30 s: no JSON line, no `Duration:` line, no timeout line, and "Run now" answered "Function invoked successfully" each time. So an empty log is evidence, not proof; the receipt in step 3 is what tells a killed run from a run that never started. The page also has a **Run now** button, which is the only supported way to invoke a scheduled function by hand (over HTTP it answers 403).
2. **Registered** — `GET /api/v1/sites/<site_id>/deploys/<deploy_id>` (authenticated; the MCP `netlify-deploy-services-reader` `get-deploy-for-site` returns the same object) carries `function_schedules: [{ name, cron }]` for the **published** deploy. If the function is missing there, the build never registered it and no amount of waiting will help.
3. **Did the work** — compaction writes its own receipt to the artifact-index store at `maintenance/media-compaction-sweep.json` (`packages/core/server/lib/media-compaction-heartbeat.ts`): `startedAtISO` before any work, `finishedAtISO` + `totals` + `resume` cursors at the end. A receipt with a start and no finish is an invocation that died mid-run — the one failure mode a log-only interface hides. `membership-sweep` (03:17, the fleet's one scheduled function with an unbroken run record) reports `media_compaction_age_hours` and `media_compaction_stale` in its daily log line, so a compaction that stops happening shows up in a log that is known to be written.
