...
| `/admin/inventory` | Owner+Admin inventory — search, preview, and act on every object, artifact and system store (T6 supersedes the owner-only `/admin/maintenance`, which now 301s here) | One search box across three collections (Objects/Artifacts/System stores) with a facet rail; row preview + Drawer inspector; bulk toolbar (role- and selection-gated) with a per-item result report; Send-to-chat handoff; collapsed "System health" diagnostics card | new `admin-inventory` (search/preview/delete-artifact/retag-artifact); existing `admin-object` verbs (`retire`/`validate`) for objects; `admin-blob-manager` + `admin-blob-store-diagnostics` for raw store reads/diagnostics and owner-only delete-blob/wipe-store/wipe-all |
