...
- **As built (T18.7, 2026-08-17) — the cross-site person seam.** `person_id` is DETERMINISTIC (`personIdForEmail` = `usr_` + base32(sha256(lower-cased e-mail))[:20], `membership/store.ts`, per §10 Q4 "yes"), so the same human resolves to the same id in every tenant's `users` store with no shared table and no identity service. A future `fleet-admin` therefore lists "Wolf: owner on drlurie / platform / fernwell / zilberman" by opening each site's store (its own credentials, its own blob namespace — nothing in `13-separation-plan.md` is contradicted; only the KEY derivation is shared law) and reading `membership/<person_id>.json`. The minimal code for that seam is `packages/core/server/lib/membership/fleet.ts` → `listMembershipsForPerson(stores[], person_id)` (pure over injected stores, tested, NO caller yet — by design). E-mail change keeps the original `person_id` (§4 `email_changed` re-indexes `by-email`), so an id minted from the first-seen e-mail stays stable across sites even after a rename on one of them; two sites that first see the person under *different* e-mails will hold two ids — the fleet-admin merges by e-mail history at read time, never by rewriting a tenant's keys. Also T18.7: each site now commits `config/membership-policy.ts` (a `MembershipPolicyOverride`, empty = fleet defaults) registered by its `policy-bindings.ts` via `setActiveMembershipPolicyProvider`; the effective policy is DEFAULT ← committed override ← store `policy.json`. The internal-only MCP tool `membership_status` (non-secret: store reachability + policy provenance) feeds the fleet probe's `membership` family.
...
**As built (T18.4, 2026-08-17) — the identity-token constraint.** GoTrue admin calls (`DELETE /admin/users/{id}`, `GET /admin/users`) need the admin token Netlify injects at `context.clientContext.identity` **only into requests that carry an Identity JWT**. The daily `membership-sweep` scheduled function has none, so it can expire invitations and purge/scrub memberships but cannot delete GoTrue identities. Therefore: `remove`/`purge` delete the identity **at request time when the Owner's request carries a token**; otherwise the delete is **queued** (`users` store key `identity-delete-queue/<person_id>.json`) and **drained at the start of the next Owner request that has a token** (`drainIdentityDeleteQueue`); the sweep only reports how many are waiting. `remove` accepts `delete_identity?: boolean` (default `policy.delete_identity_on_remove = true`, §9-3). OAuth revocation is by the new `oauth/by-subject/<email>/…` index written on every mint (records minted before T18.4 are not indexed until they rotate — access tokens live 1 h, refresh tokens rotate on use). Suspend and remove both revoke OAuth grants and force-release held locks (`lock_forced_on_offboarding` history entry, actor = the Owner, `on_behalf_of` = the person). Effective JWT lockout stays ≤ 1 h (no server-side GoTrue logout) but every function re-resolves roles per call, so a suspended member cannot act.
