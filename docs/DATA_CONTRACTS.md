...
> Evidence class for every row: the shape this repo *sends or expects* is LOCAL EVIDENCE @ `420afbd`; the sibling system's actual contract is EXTERNAL UNVERIFIED, except kugel-data rows, which are EXTERNAL VERIFIED @ `6c9c712` (`ARCHITECTURE.md` §13).
...
| **GitHub Git Data API — refs** (`GET /repos/{repo}/git/ref/heads/{branch}`, the same endpoint family the committer uses) + **REST compare** (`GET /repos/{repo}/compare/{base}...{head}`) | GitHub | `server/lib/production-release.ts:resolveBranchHeadCommit`, `isCommitAncestorOrEqual` | HTTPS | Silent `undefined` → `commit_unresolved`; compare failure → not confirmed. |
...
| Examples-job trigger token (#692) | `server/lib/visual-standard-examples-jobs.ts` → `server/functions/visual-standard-examples-background.ts` | object-verb / `admin-object` write → background lambda | One-shot `trigger_token`, consumed on a successful run; job record `visual-standard-examples/<visual_standard_id>.json` in the **`artifact-index`** store (no new namespace); statuses `pending → ready \| partial \| failed` (`examplesJobStatusSchema`); `trigger: 'mcp' \| 'browser'` (`tests/netlify/visual-standard-examples-job.test.ts`) |
| `visual_standard` reference ids (#690) | `packages/core/lib/object-ids-mint.ts:156-164` (`ref_<8 hex>`, `refIdSchema` in `schema/bodies/visual-standard-v1.ts:63`); minted in `object-verbs.ts:631-663` for `set_visual_standard_fields.references[]` | patch op → stored body | Caller-supplied ids are kept; duplicates throw `PatchApplyError('invalid_body')` → 422. Contract constraints `reference_ids` (`enforced_live: true`) and `reference_import_request_ids` (`enforced_live: false`, `req_visref_<site>_<yyyymmdd>_<nn>`) in `packages/core/lib/registry/object-contract.ts:850-865` |
| Drill traffic tag (#689) | `packages/core/cli/capture/test-traffic-header.mjs` (`x-trk-test: 1`) | capture drills / e2e → tenant site | **One-sided today:** `track-ingest.ts` neither reads nor forwards request headers to the sink (`forwardToSink` sends only `Content-Type` + `Authorization`), and kugel-data @ `6c9c712` has no handling for it — see `KNOWN_ISSUES.md` #63 |
