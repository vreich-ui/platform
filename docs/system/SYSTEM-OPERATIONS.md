...
| platform `main` | `CMS_AGENT_BOUNDS`; `TRACKING_EVENT_KINDS` and `trackingPropsSchema` (does `exposure`/`variant_id` exist now?); `own-tracker-stats.ts` query shape (`days` vs `from/to`); any `/rollups`, `/weights`, `/export` consumer; `allowedArtifactReferenceKeys`; grant shape; `commerce-events.ts` `kind`; checkout id generators; `tracking-dims-push.mjs`; hooks' consumers (`object_publish` inputs, `producer`); the tool count (101) and any new tool platform calls on CMS-Agent (the Insights tab and the analytics MCP tools call CMS-Agent too — SYSTEM-KNOWN-ISSUES.md S-07) |
